...
```{r echo= TRUE}
kable(bias,row.names=FALSE) 
```
-We can see that the bias of the restricted estimator increases as the size of beta_2 in our DGP increases--in other words, our estimate of alpha is picking up some of the effect of beta_2, since x_1 and x_2 are strongly correlated.
...
ci <- cbind(ci$beta,ci$n,ci$`long alpha in CI`,ci$`short alpha in CI`,ci$`everday in CI`)
...
```
-This shows the same distortion of the estimator for alpha that we saw in the previous slide.
